...
main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "motorcontrolabd");//初始化ros节点
	pthread_t threadid0;
	int ret0;
	...
	pthread_t threadid1;
	int ret1;
	ret1 = pthread_create(&threadid1,NULL,pub_status, &cache);//启动线程
	...
	//下面测试力控效果，输入固定的控制目标
	float T_tar = 0.5; //控制末端输出力为10N，则弹簧末端输出扭矩为0.3Nm
	delta_theta_d1 = 0;delta_theta_d2=0;
	const int32_t pos_Limit1 = cnt_per_circle * 3; //设置位置上下限,2圈
	const int32_t pos_Limit2 = cnt_per_circle * 3; //设置位置上下限,2圈
	...
	while (ros::ok())
	{
		...
		while (ros::ok())	//引入对电机位置的判断，消除电机位置奇异点
		{
			...
			if (abs(theta_m1 - theta_m1_prev) < 5)//非奇异点
			{
				break;
			}
		...
		// int32_t pos1 = (int32_t)(((theta_l1 - theta_l_i1) + delta_result + delta_theta_d1) / 360.0 * cnt_per_circle + theta_m_i1);
		int32_t pos1 = (int32_t)((delta_result + delta_theta_d1) / 360.0 * cnt_per_circle + theta_m_i1);
		printf("电机1：实测拉力值:%f\t理论角度偏差:%f\t实际角度偏差:%f\t电机实际转角:%f\t电机指令转角:%d\r\n",Trr_ad, delta_theta_d1, delta_theta_r1, pos1);
